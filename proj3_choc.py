...
def process_command(command):
    ...
    if parameters[0] == "bars":
        ...
        used_commands = ['AS c1 ON Bars.CompanyLocationId = c1.Id JOIN Countries AS c2 ON Bars.BroadBeanOriginId = c2.Id', 'ORDER BY Bars.Rating DESC', 'LIMIT 10']
        for parameter_per_command in parameters[1:]:
            if "=" in parameter_per_command:
                ...
                if parameter[0] in ["sellcountry", "sourcecountry"]:
                    ...
                    used_commands[0] = "AS c1 ON {} = '{}' AND {} = c1.Id JOIN Countries as c2 ON c2.Id = {}".format(columns[0], parameter[1], columns[1], columns[1])
        ...
        statement_bars = 'SELECT Bars.SpecificBeanBarName, Bars.Company, c1.EnglishName, Bars.Rating, ' \
                         'Bars.CocoaPercent, c2.EnglishName FROM Bars JOIN Countries {} {} {}'.format(used_commands[0], used_commands[1], used_commands[2])
    ...
    if parameters[0] == "companies":
        used_commands = ["AVG(Bars.Rating) AS average", "AS c1 ON c1.Id = Bars.CompanyLocationId", "ORDER BY average DESC", "LIMIT 10"]
        column_name = {
        ...
        for parameter_per_command in parameters[1:]:
            if "=" in parameter_per_command:
                ...
                if parameter[0] == "country":
                    columns = column_name[parameter[0]]
                    used_commands[1] = "AS c1 ON {} = '{}' AND {} = c1.Id JOIN Countries as c2 ON c2.Id = {}".format(columns[0], parameter[1], columns[1], columns[1])
        ...
        statement_companies = 'SELECT Bars.Company, c1.EnglishName, {} FROM Bars JOIN Countries {} GROUP BY Bars.Company HAVING COUNT(Bars.Company) > 4 {} {}'.format(used_commands[0], used_commands[1], used_commands[2], used_commands[3])
    ...
    if parameters[0] == "countries":
        used_commands = ["AVG(Bars.Rating) AS average", "AS c1 ON", "c1.Id = Bars.CompanyLocationId", "Bars.CompanyLocationId", "average", "ORDER BY average DESC", "LIMIT 10"]
        column_name = {
        ...
        for parameter_per_command in parameters[1:]:
            ...
            if "bars_sold" == parameter_per_command:
                ...
                # print("error")
                continue
        statement_countries = 'SELECT c1.EnglishName, c1.Region, {} FROM Bars JOIN Countries {} {} GROUP BY {} HAVING {} > 4 {} {}'.format(used_commands[0], used_commands[1], used_commands[2], used_commands[3], used_commands[4], used_commands[5], used_commands[6])
    ...
    if parameters[0] == "regions":
        used_commands = ["AVG(Bars.Rating) AS average", "Bars.CompanyLocationId", "average", "ORDER BY average DESC", "LIMIT 10"]
        column_name = {
            "sellers": ("c1.Region", "Bars.CompanyLocationId"),
            "sources": ("c1.Region", "Bars.BroadBeanOriginId")
        }
        for parameter_per_command in parameters[1:]:
            ...
            if "bars_sold" == parameter_per_command:
                ...
                # print("error")
                continue
        statement_regions = 'SELECT c1.Region, {} FROM Bars JOIN Countries AS c1 ON c1.Id = {} GROUP BY c1.Region HAVING {} > 4 {} {}'.format(
            used_commands[0], used_commands[1], used_commands[2], used_commands[3], used_commands[4])
...
def interactive_prompt():
    ...
    bars_commands = ["bars", "sellcountry", "sourcecountry", "sellregion", "sourceregion", "ratings", "cocoa", "top", "bottom"]
...
if __name__ == "__main__":
    ...
    # print(process_command('ratings sellers regions top=10'))
    interactive_prompt()
